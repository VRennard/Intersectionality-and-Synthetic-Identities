#!/usr/bin/env python3
"""
augment_party_doubles.py

Adds all Political Party x {Age, Gender, Race, Income} double profiles
...
Usage:
    python augment_party_doubles.py --api-key sk-... --waves 26 27 29 32
    python augment_party_doubles.py --api-key sk-... --waves all
"""
...
PARTY_DIM   = "Political Party"
OTHER_DIMS  = ["Age", "Gender", "Race", "Income"]
...
def build_party_double_profiles(wave):
    party_vals = get_vals(PARTY_DIM, wave)
    profiles = []
    for other in OTHER_DIMS:
        other_vals = get_vals(other, wave)
        for pv, ov in product(party_vals, other_vals):
            profiles.append(DemographicProfile(
                features=[f"{PARTY_DIM} {pv}", f"{other} {ov}"]
            ))
    return profiles
...
def main():
    ...
    print(f"Party doubles augment | waves: {waves} | model: {args.model}")
    ...
    for wave in waves:
        ...
        profiles = build_party_double_profiles(wave)
        total_calls = len(profiles) * len(questions)
        print(f"  {len(profiles)} party-double profiles x {len(questions)} questions = {total_calls} calls")
