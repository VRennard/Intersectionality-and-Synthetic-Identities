...
WAVE = "26"
TAGS = ["claude-haiku-4-5-20251001", "claude-haiku-4-5-20251001_promptI", "claude-haiku-4-5-20251001_promptC"]
...
def score_tag(tag):
    ...
    if tag == "claude-haiku-4-5-20251001":
        BASE_SINGLES = {k: v for k, v in llm.items() if len(k) == 1}
    else:
        # instructed variants are pairs-only; singles come from baseline
        for k, v in (BASE_SINGLES or {}).items():
            llm.setdefault(k, v)
...
names = {"claude-haiku-4-5-20251001": "baseline (no instruction)",
         "claude-haiku-4-5-20251001_promptI": "instructed integration",
         "claude-haiku-4-5-20251001_promptC": "chain-of-thought first"}
for t in TAGS:
