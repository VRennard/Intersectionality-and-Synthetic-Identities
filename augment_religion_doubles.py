#!/usr/bin/env python3
"""
augment_religion_doubles.py

Adds all Religion x {Age, Gender, Race, Income, Party} double profiles
across specified waves, appending to existing JSONL files.
Already-completed (profile, question) pairs are skipped automatically.

Usage:
    python augment_religion_doubles.py --api-key sk-... --waves 26 27 29 32
    python augment_religion_doubles.py --api-key sk-... --waves all
"""
...
RELIGION_DIM = "Religion"
OTHER_DIMS   = ["Age", "Gender", "Race", "Income", "Political Party"]
...
def build_religion_double_profiles(wave):
    religion_vals = get_vals(RELIGION_DIM, wave)
    profiles = []
    for other in OTHER_DIMS:
        other_vals = get_vals(other, wave)
        for rv, ov in product(religion_vals, other_vals):
            profiles.append(DemographicProfile(
                features=[f"{RELIGION_DIM} {rv}", f"{other} {ov}"]
            ))
    return profiles
...
def main():
    ...
    print(f"Religion doubles augment | waves: {waves} | model: {args.model}")
    ...
    for wave in waves:
        ...
        profiles = build_religion_double_profiles(wave)
        total_calls = len(profiles) * len(questions)
        print(f"  {len(profiles)} religion-double profiles x {len(questions)} questions = {total_calls} calls")
        ...
        added = errors = 0
        for i, (profile, q) in enumerate(to_do, 1):
            try:
            ...
            added += 1
            if added % 100 == 0:
                print(f"  [{added}/{len(to_do)}] last: {profile.features[0]}, {profile.features[1]}")
